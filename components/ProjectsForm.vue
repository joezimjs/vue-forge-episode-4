<script setup lang="ts">
	...
	// handle form submit
	const submitForm: SubmissionHandler = async () => {
		...
		if (requestKey) {
			const project = await createProjectInDB({
				...
				excerpt: form.description.length <= 130 ? form.description : `${form.description.substring(0, 130)} ...`
			})

			useAlerts().success('Project created', { title: 'YAY! 🎉' })
			navigateTo(`/projects/${project.uuid}`)
		} else {
			useAlerts().error('There was an error creating your project.', { title: 'DANG IT!' })
		}
	}

	function failedSubmit() {
		useAlerts().error('Please fix the invalid fields', { title: 'Validation Error' })
	}
</script>
...
<template>
	<div class="w-full max-w-5xl mx-auto mb-20">
		...
		<div class="grid grid-cols-12 gap-8">
			<Form @submit="submitForm" @invalid-submit="failedSubmit" class="w-full col-span-8" :validation-schema="validationSchema">
				<FormField
